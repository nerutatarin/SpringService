...
import configurations.ClientConfig;
import okhttp3.*;
...
public abstract class BasicClient {
    ...
    protected <T> T getResponse(Call<Weather> call) {
        return getOrThrowException(call, ResponseBody::string);
    }

    protected <T, U extends retrofit.Response> T getResponse(Call<T> call, Class<U> errorModel) {
        return getOrThrowException(call, errorBody -> getApiErrorText(errorBody, errorModel));
    }

    private <T extends retrofit.Response> String getApiErrorText(ResponseBody body, Class<T> model) throws IOException {
        String payload = body.string();
        if (!Utils.isBlankString(payload)) {
            try {
                retrofit.Response response = gson.fromJson(payload, model);
    ...
    private <T> T error(Response<T> response, ErrorMessageExtractor extractor) throws IOException {
        ...
        if (errorBody != null) {
            message.append(": ").append(extractor.extract(errorBody));
        }
